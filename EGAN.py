...
for base_epoch_i in range(base_epochs):         
    ...
    if (base_epoch_i+1) % 10 == 0:
        ...
        with torch.no_grad():
            ...
            torch.save(net.state_dict(), './model_pth/' + NOTE + '/' + str(int(base_epochs)) + "_final.pth") #final path
            torch.save(G.state_dict(), net_pth_G)
            torch.save(E.state_dict(), net_pth_E)
            
################### visualization            
        model = Generator(img_ch=1, output_ch=1,output_process = '0to1')  #before binarization (in align with the training net)
        model_binary = Generator(img_ch=1, output_ch=1,output_process = 'sign0.5')#post training binarization
